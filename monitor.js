...
class TokenMonitor {
  ...
  async _evaluateAll() {
    for (const [addr, state] of this.tokens.entries()) {
      ...
      // Exclude the last (potentially incomplete) candle from EMA calculation.
      // The final bucket is still forming; including it causes EMA to flicker
      // on every poll tick instead of updating once per closed bar.
